...
<head>
    ...
    <!-- Style import -->
    <link rel="stylesheet" href="complaintsTimeLinePage/complaintsTimeLine.css">

    <!-- Favicon import -->
    <link rel="icon" href="assets/favicon_io/logoGuia.svg">

    <title>Lista de denúncias | Econnect</title>
...
<body>
    ...
    <main>
        ...
        <section id="listAllComplaints">

            <?php 
                if(!empty($allComplaints)){ 
                    for($i = 0; $i < count($allComplaints); $i++) {
                        $queryUserCreateComplaints = "SELECT * FROM users WHERE Id_user = :userId";
                        $stmt = $connection->prepare($queryUserCreateComplaints);

                        $stmt->bindValue(':userId', $allComplaints[$i]->User_id);
        
                        $stmt->execute();
                        $userCreateComplaints = $stmt->fetchAll(PDO::FETCH_OBJ);
            ?>

                <div class="denuncePost">
                    <section class="garbagePhoto skeleton" <?php if(!empty($allComplaints[$i]->Photo_path)) { ?> style="background-image: url(mediasComplaints/<?php echo $allComplaints[$i]->Photo_path; }?>);">
                        <div class="engagementCount">
                            <div class="counter">
                                <h4>
                                    <?php 
                                        $current = explode('/', $allComplaints[$i]->Supporters)[0]; 

                                        $goal = explode('/', $allComplaints[$i]->Supporters)[1]; 
                    ...
                    <div class="panel skeleton">
                        <div class="creatorUser">
                            <img src="<?php if(!empty($userCreateComplaints[0]->Photo_path)) {echo 'mediaFilesDB/' . $userCreateComplaints[0]->Photo_path; }else { echo 'assets/midia/defaultUserPhoto.jpg'; } ?>" alt="Imagem do usuário">
                            <h4><?php if(isset($userCreateComplaints[0]->Name_user)) {echo explode(' ', $userCreateComplaints[0]->Name_user)[0]; if(isset(explode(' ', $userCreateComplaints[0]->Name_user)[1])) {echo ' ' . explode(' ', $userCreateComplaints[0]->Name_user)[1]; }}else {echo '<i>Desconhecido</i>';} ?></h4>
                        </div>

                        <div class="options">
                            <a href="database/controller.php?action=complaintsDetails-<?php echo $allComplaints[$i]->Id_complaint; ?>">
                                <button class="option">
                                    <svg xmlns="http://www.w3.org/2000/svg" width="28" height="28" viewBox="0 0 28 28" fill="none">
                                    <path d="M14 5.83398V22.1673" stroke="#00F0FF" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"/>
                                    <path d="M5.83331 14H22.1666" stroke="#00F0FF" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"/>
                                    </svg>
                                </button>
                            </a>
                            ...
                            <?php 
                                if(isset($user)) {
                                    if(isset($user->Complaints_support)) {
                                        $allSupportComplaints = explode(',', $user->Complaints_support);
                                    }
                                ?>

                                <a href="<?php 
                                        if(isset($allSupportComplaints) && !in_array($allComplaints[$i]->Id_complaint, $allSupportComplaints)) { echo "database/controller.php?action=support-" . $allComplaints[$i]->Id_complaint; 
                                    } else{
                                            echo '#';
                                        } 
                                    ?>" class="option <?php if(isset($allSupportComplaints) && in_array($allComplaints[$i]->Id_complaint, $allSupportComplaints)) {echo 'disabled';}?>">

                                    <svg xmlns="http://www.w3.org/2000/svg" width="28" height="28" viewBox="0 0 28 28" fill="none">
                                        <g clip-path="url(#clip0_772_1975)">
                                        <path d="M26.8334 7L15.75 18.0833L9.91669 12.25L1.16669 21" stroke="url(#paint0_linear_772_1975)" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"/>
                                        <path d="M20.125 6.125H27.125V13.125" stroke="url(#paint1_linear_772_1975)" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"/>
                                        </g>
                                        <defs>
                                        <linearGradient id="paint0_linear_772_1975" x1="14" y1="7" x2="14" y2="21" gradientUnits="userSpaceOnUse">
                                        <stop stop-color="#06B802"/>
                                        <stop offset="1" stop-color="#15FF10"/>
                                        </linearGradient>
                                        <linearGradient id="paint1_linear_772_1975" x1="23.625" y1="6.125" x2="23.625" y2="13.125" gradientUnits="userSpaceOnUse">
                                        <stop stop-color="#06B802"/>
                                        <stop offset="1" stop-color="#15FF10"/>
                                        </linearGradient>
                                        <clipPath id="clip0_772_1975">
                            ...
                            <?php } else if(!isset($user)) { ?>
                                <a href="database/controller.php?action=support-<?php echo $allComplaints[$i]->Id_complaint; ?>" class="option disabled">
                                    <svg xmlns="http://www.w3.org/2000/svg" width="28" height="28" viewBox="0 0 28 28" fill="none">
                                        <g clip-path="url(#clip0_772_1975)">
                                        <path d="M26.8334 7L15.75 18.0833L9.91669 12.25L1.16669 21" stroke="url(#paint0_linear_772_1975)" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"/>
                                        <path d="M20.125 6.125H27.125V13.125" stroke="url(#paint1_linear_772_1975)" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"/>
    ...
    <script src="assets/JS/complaintsTimeLine.js"></script>
