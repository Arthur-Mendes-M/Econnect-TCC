...
<head>
    ...
    <!-- Style import -->
    <link rel="stylesheet" href="perfilPage/perfil.css">

    <!-- Favicon import -->
    <link rel="icon" href="assets/favicon_io/logoGuia.svg">
    

    <title>Perfil | Econnect</title>
...
<body>
    ...
    <header>
        ...
        <div id="classification">
            <div id="ratingStars" class="skeleton">
                <?php if(isset($_SESSION['user_id'])) {
                        
                    if(count($allComplaintsUser) < 8) { ?>
                        <svg class="star active" xmlns="http://www.w3.org/2000/svg" width="25" height="24" viewBox="0 0 25 24" fill="none">
                        <path d="M12.4365 2L15.5265 8.26L22.4365 9.27L17.4365 14.14L18.6165 21.02L12.4365 17.77L6.25652 21.02L7.43652 14.14L2.43652 9.27L9.34652 8.26L12.4365 2Z" stroke="white" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"/>
                        </svg>
                        ...
                        <svg class="star" xmlns="http://www.w3.org/2000/svg" width="25" height="24" viewBox="0 0 25 24" fill="none">
                        <path d="M12.4365 2L15.5265 8.26L22.4365 9.27L17.4365 14.14L18.6165 21.02L12.4365 17.77L6.25652 21.02L7.43652 14.14L2.43652 9.27L9.34652 8.26L12.4365 2Z" stroke="white" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"/>
                        </svg>
                    ...
                    <?php }else if(count($allComplaintsUser) >= 9 && count($allComplaintsUser) <= 15) { ?>
                        <svg class="star active" xmlns="http://www.w3.org/2000/svg" width="25" height="24" viewBox="0 0 25 24" fill="none">
                            <path d="M12.4365 2L15.5265 8.26L22.4365 9.27L17.4365 14.14L18.6165 21.02L12.4365 17.77L6.25652 21.02L7.43652 14.14L2.43652 9.27L9.34652 8.26L12.4365 2Z" stroke="white" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"/>
                        </svg>
                    ...
                    <?php }else if(count($allComplaintsUser) >= 16) { ?>
                        <svg class="star active" xmlns="http://www.w3.org/2000/svg" width="25" height="24" viewBox="0 0 25 24" fill="none">
                        <path d="M12.4365 2L15.5265 8.26L22.4365 9.27L17.4365 14.14L18.6165 21.02L12.4365 17.77L6.25652 21.02L7.43652 14.14L2.43652 9.27L9.34652 8.26L12.4365 2Z" stroke="white" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"/>
                        </svg>
                ...
                <h3 class="skeleton" 
                    title="
                        <?php if(isset($_SESSION['user_id'])) {
                            ...
                            if(intval($user->Support) < 8) {
                                echo 'Pessoas prudentes evitam perigos e são precavidas. É a qualidade de quem busca evitar consequências ruins através de ações.';
                            }else if(intval($user->Support) >= 9 && intval($user->Support) <= 15) {
                                echo 'O altruísmo é a capacidade de não ser egoísta e buscar ajudar o próximo em diferentes situações.';
                            }else if(intval($user->Support) >= 16) {
                                echo 'Uma pessoa responsável cumpre com suas obrigações e tarefas, não importa o que aconteça.';
                            }

                        }else {
                            echo 'Faça login para começar a ajudar o mundo através de denúncias';
                        } ?>
                    "
                >
                    <?php if(isset($_SESSION['user_id'])) {
                        ...
                        if(count($allComplaintsUser) < 8) {
                            echo 'Prudente';
                        }else if(count($allComplaintsUser) >= 9 && count($allComplaintsUser) <= 15) {
                            echo 'Altruísta';
                        }else if(count($allComplaintsUser) >= 16) {
                            echo 'Responsavel';
                        }
    ...
    <main>
        <section id="userInfos">
            ...
            <div id="classification">
                ...
                <span class="activityClass">
                    <h3 class="skeleton" 
                        title="<?php if(isset($_SESSION['user_id'])) {
        ...
        <section id="activityHistory">
            <div class="communitySupportData">    
                <div class="engagements skeleton">
                    <h4 title="Quantidade de vezes que você apoiou uma denúnica criada">Engajamentos:</h4>
                    <h4 class="engagementsCounter">
                        <?php 
                            if(isset($user) && isset($user->Support)) {echo $user->Support;} else {echo '0';}
                        ?>
                    </h4>
                </div>

                <div class="countPublications skeleton">
                    <h4>Denúncias:</h4>
                    <h4 class="publicationsCounter">
                        <?php if(isset($allComplaintsUser)) {echo count($allComplaintsUser);} else {echo '0';}?>
                    </h4>
                </div>
            </div>
        </section>
        ...
        <section id="publications">
            ...
            <?php 
            if(!empty($allComplaintsUser)){ 
                    for($i = 0; $i < count($allComplaintsUser); $i++) {
                        if(empty($allComplaintsUser[$i]->Photo_path)) {
                ?>

                <img class="skeleton" src="mediasComplaints/DefaultComplaintPhoto.jpg" alt="Image">

            <?php }else { ?>

                <img class="skeleton" src="mediasComplaints/<?php echo $allComplaintsUser[$i]->Photo_path;?>" alt="Image">

            <?php } } } else { ?> 
                
                <div class="notPhotos">
                    <h2>Você ainda não possui denúnicias :(</h2>
                    <p>Vá até a <a href="denounce.php" class="link">página de denúncias</a> e denuncie um local com acúmulo indevido de resíduos.</p>
                </div>
